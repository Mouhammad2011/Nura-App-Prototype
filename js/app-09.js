...
function v9_patchSendMsg(){
  ...
  window.sendMsg = async function(){
    ...
    if(mode === 'code'){
      ...
      try{
        const sysPr = 'You are Neuro Code 1.0, an expert AI coding assistant. Unless the user explicitly requests another language or file type, build one complete self-contained HTML file with embedded CSS and JavaScript. Write ONLY the complete code — no explanations, no markdown backticks, no preamble. Just the raw code starting immediately. Max 50000 characters.';
        const res = await fetch('https://api.groq.com/openai/v1/chat/completions',{
    ...
    _configFetchInFlight = (async () => {
      ...
      try{
        ...
        if(snap.exists){
          ...
          const groq = d.groqKey || d.groq_api || d.groqApi || d.groqAPI || d['groq api'] || d['groq Api'] || d.groq;

...
function checkGreeting(){
  ...
  else if(h >= 5 && h < 7)   msg = "Early bird grind â€” respect. Let's go.";
  else if(h >= 7 && h < 10)  msg = "Morning session activated. Sharp mind incoming.";
  else if(h >= 10 && h < 12) msg = "Peak focus window. Make it count.";
  else if(h >= 12 && h < 14) msg = "Post-lunch? Power through â€” you've got this.";
  else if(h >= 14 && h < 17) msg = "Afternoon flow â€” the grind never stops.";
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   V9.4 â€” QUIZ VALIDATION
   Replace alert() with custom error div
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
const QUIZ_ERROR_MSGS = [
  "Sorry, we still don't support geniuses â€” but we can take up to 20.",
  ...
  "We checked â€” your brain can only handle 20. Cap reached.",
...
/* Swap quiz count SELECT â†’ number INPUT */
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   V9.8 â€” NURA MIND MODES: Deep Search / Chat
...
function v9_injectNuraMindModes(){
  ...
  switcher.innerHTML = `
    ...
    <div id="nura-model-dropdown" style="display:none;position:absolute;top:calc(100% + 6px);left:0;background:var(--bg2);border:1px solid var(--glass-border);border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.35);z-index:500;min-width:220px;overflow:hidden;animation:loginIn .18s cubic-bezier(.2,.9,.25,1)">
      <div onclick="v9SetNuraMode('chat')" id="nura-tab-chat" style="padding:10px 14px;cursor:pointer;display:flex;align-items:center;gap:10px;transition:background .15s" class="model-opt active-model" onmouseover="this.style.background='var(--bg3)'" onmouseout="this.style.background=''">
        ...
        <div><div style="font-size:12px;font-weight:600;color:var(--text)">Nura Flow 1.0</div><div style="font-size:10px;color:var(--text3)">Conversational Â· Socratic teaching</div></div>
      </div>
      <div onclick="v9SetNuraMode('research')" id="nura-tab-research" style="padding:10px 14px;cursor:pointer;display:flex;align-items:center;gap:10px;transition:background .15s" class="model-opt" onmouseover="this.style.background='var(--bg3)'" onmouseout="this.style.background=''">
        ...
        <div><div style="font-size:12px;font-weight:600;color:var(--text)">Nura Quest 1.0</div><div style="font-size:10px;color:var(--text3)">Deep research Â· Cited sources</div></div>
      </div>
      <div id="nura-tab-code" style="padding:10px 14px;display:flex;align-items:center;gap:10px;position:relative" class="model-opt">
        ...
        <div><div style="font-size:12px;font-weight:600;color:var(--text)">Neuro Code 1.0 <span style="font-size:9px;background:rgba(168,85,247,.15);border:1px solid rgba(168,85,247,.3);color:#c084fc;padding:1px 6px;border-radius:100px;margin-left:4px">PRO</span></div><div style="font-size:10px;color:var(--text3)">AI coding assistant Â· File output</div></div>
      </div>
    </div>
  `;
  ...
  window.v9SetNuraMode = function(mode){
    ...
    if(mode === 'code'){
      ...
      if(p.plan !== 'pro'){
        nuraDialog && nuraDialog({title:'Neuro Code â€” Pro Only',body:'Upgrade to Nura NeuroMind to unlock the AI coding assistant with file output.',icon:'info',confirmText:'Upgrade',cancelText:'Maybe later'}).then(r=>{ if(r) { closeSettingsModal && closeSettingsModal(); handleUpgradeClick && handleUpgradeClick(); } });
        return;
      }
    }
    _nuraMindMode = mode;
    ...
    const tags = {chat:'<div class="mode-dot"></div>NURA FLOW Â· SOCRATIC', research:'<div class="mode-dot"></div>NURA QUEST Â· DEEP RESEARCH', code:'<div class="mode-dot"></div>NEURO CODE Â· AI DEVELOPER'};
  ...
  window.sendMsg = async function(){
    ...
    if(mode === 'code'){
      ...
      if(!codeKey && typeof window._fetchConfigKey === 'function'){
        // Config key may not have loaded yet â€” fetch it live instead of failing immediately
        await window._fetchConfigKey();
        codeKey = getApiKey();
      }
      if(!codeKey){ v9_addChatMsg('ai','Neuro Code is unavailable â€” no API key configured. Contact support.'); return; }
      ...
      v9_addChatMsg('ai','<span id="'+codeTypingId+'"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="animation:spin 1s linear infinite"><line x1="12" y1="2" x2="12" y2="6"/><line x1="12" y1="18" x2="12" y2="22"/><line x1="4.93" y1="4.93" x2="7.76" y2="7.76"/><line x1="16.24" y1="16.24" x2="19.07" y2="19.07"/><line x1="2" y1="12" x2="6" y2="12"/><line x1="18" y1="12" x2="22" y2="12"/></svg> Neuro Code is writing your fileâ€¦</span>', 'html');
      try{
        const sysPr = 'You are Neuro Code 1.0, an expert AI coding assistant. Write ONLY the complete code â€” no explanations, no markdown backticks, no preamble. Just the raw code starting immediately. Max 50000 characters.';
        const res = await fetch('https://api.groq.com/openai/v1/chat/completions',{
    ...
    if(!window.getApiKey || !window.getApiKey()){
      v9_addChatMsg('ai','<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> No API key â€” contact support.'); return;
    }
...
function v9_addChatMsg(role, text, parseSourceChips){
  ...
  if(parseSourceChips && role === 'ai'){
    // Strip [SOURCE: url] tags â€” collect into side panel, never show inline
    ...
    // Raw HTML allowed â€” used for rich AI messages (Neuro Code file cards etc.)
    htmlContent = text;
  } else {
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   V9.10 â€” STATS DAILY GOALS WIDGET
...
function v9_buildStatsDailyGoals(){
  ...
  wrap.innerHTML = `
    <div style="font-size:10px;color:var(--text3);letter-spacing:2.5px;text-transform:uppercase;margin-bottom:12px">Daily Goals â€” This Week</div>
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   V9 BOOT â€” runs on DOMContentLoaded + load
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
function renderSourcesList(){
  ...
  if(_nuraSources.length === 0){
    ul.innerHTML = '<li style="padding:14px 2px;font-size:12px;color:var(--text3)">No sources yet â€” use NuraSearch and ask a question.</li>';
    return;
  }
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   ...
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
(function(){
  // Fetch from Firestore config/app â€” works for guests + signed-in users
  let _configFetchInFlight = null;
  async function fetchConfigKey(){
    // De-dupe overlapping calls â€” return the same in-flight promise
...
window.loadChatHistoryPanel = async function(){
  ...
  if(!listEl) return;
  // Each call gets its own token â€” only the LATEST call is allowed to render its result
  const myToken = ++_chatHistLoadToken;
  listEl.innerHTML = '<div style="font-size:11px;color:var(--text3);padding:12px 0;text-align:center">Loadingâ€¦</div>';
  ...
  // A newer call started while we were awaiting â€” discard this stale result
...
window.restoreChatSession = function(id, messagesJson){
  ...
  saveChatHistory(chatHistory);
  // Render messages â€” user can continue chatting after this
...
/* â”€â”€ AI title generator via Groq â”€â”€ */
...
// Override startNewConversation â€” snapshot BEFORE clearing so we don't lose data
...
window.startNewConversation = function(){
  ...
  if(snapshot){
    ...
    _generateChatTitle(snapshot).then(async aiTitle=>{
      ...
      // Single refresh, only after BOTH writes are truly finished â€” no race
...
window.processPayment = async function(){
  ...
  if(hasError) return;
  // Demo â€” mark user as pro in Firestore
  const user = typeof _auth !== 'undefined' ? _auth.currentUser : null;
  if(user){
    try{
...
window.openSettingsModal = function(){
  ...
  const user = typeof _auth!=='undefined' ? _auth.currentUser : null;
  if(user){
    const verEl=document.getElementById('s-email-verified'); if(verEl) verEl.textContent = user.emailVerified?'Email verified âœ“':'Not verified';
    const lastEl=document.getElementById('s-last-login'); if(lastEl) lastEl.textContent = user.metadata?.lastSignInTime||'â€”';
  }
  showSettingsTab('profile');
};
...
window.showPfpCropEditor = function(src){
  ...
  overlay.innerHTML = `
    <div style="background:var(--bg2);border:1px solid var(--glass-border);border-radius:18px;padding:24px;text-align:center;width:min(340px,92vw)">
      ...
      <div style="font-size:11px;color:var(--text3);margin-bottom:16px">Drag to reposition Â· Scroll to zoom</div>
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   OSCILLOGRAM â€” dashboard waveform visualiser
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
(function(){
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   NURA LAMP â€” rotatable 3D cube (drag + idle spin)
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
(function(){

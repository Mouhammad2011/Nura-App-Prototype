...
const _calYearRange = 10;             // Â±10 years from today
...
function getStudiedDates(){
  /* We sync with the week-bar structure â€” bars that are non-zero height have study data */
  const set = new Set();
  const today = new Date();
...
/* Mini preview â€” 7 coloured dots in the Stats widget */
function calRenderMiniPreview(){
  ...
  if(widgetSub) widgetSub.textContent = goalCount > 0
    ? `${goalCount} goal${goalCount>1?'s':''} set this month Â· Click to manage`
    : 'Click to open Â· Track goals & important days';
}
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   ...
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
const QUIZ_ERR_MSGS = [
  "Sorry, we don't support geniuses yet â€” but we can take up to 20.",
  ...
  "We checked â€” your brain can handle 20. That's the cap.",
...
function v9f_checkGreeting(){
  ...
  const msgs = {
    ...
    earlyBird:"Early bird grind â€” respect. Let's go.",
    morning: "Morning session activated. Sharp mind incoming.",
    midMorn: "Peak focus window. Make it count.",
    afterLunch:"Post-lunch? Power through â€” you've got this.",
    afternoon:"Afternoon flow â€” the grind never stops.",
